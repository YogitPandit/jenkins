...
app.controller('subsubCategoryController', ['$scope', 'SubsubCategoryService', 'CategoryService', 'SubCategoryService', "$filter", "$http", "ngTableParams", '$modal', 'FileUploader', function ($scope, SubsubCategoryService, CategoryService, SubCategoryService, $filter, $http, ngTableParams, $modal, FileUploader) {
    ...
    $scope.exportData = function () {
        ...
        var url = serviceBase + "api/SubsubCategory/export";
        $http.get(url).then(function (results) {

            $scope.storesitem = results.data;

            alasql('SELECT SubsubCategoryid,SubsubcategoryName,BaseCategoryId,Categoryid,SortOrder,IsPramotional,Type,Code,CategoryName,SubCategoryId,SubcategoryName,CreatedDate,UpdatedDate,CreatedBy,UpdateBy,LogoUrl,[IsActive],[Deleted],CommisionPercent,IsExclusive INTO XLSX("SubSubCategory.xlsx",{headers:true}) FROM ?', [$scope.storesitem]);
        }, function (error) {
        });
    };
    //............................Exel export Method.....................//
    ...
    //Get sub sub category data in grid
    $scope.subsubcats = [];
     ...
     $scope.AddSubsubCategorys = function (data) {
         ...
         .success(function (data) {
             ...
             if (data.Message == "Successfully") {
                 debugger;
                 alert("Sub Sub Category Add Successfully!")
                 ...
                 alert("Sub Sub Category Not Add!Some Error Occure!")
                 $modalInstance.close(data);
             }
             if (data.id == 0) {
    ...
    $scope.PutSubsubCategory = function (data) {
        ...
        if ($scope.uploadedfileName == null || $scope.uploadedfileName == '') {
            ...
            .success(function (data) {
                ...
                if (data.Message == "Successfully") {
                    debugger;
                    alert("Sub Sub Category Update Successfully!")
                    $modalInstance.close(data);
                    window.location.reload();
                }
