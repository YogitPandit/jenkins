...
app.controller('Comparison1Ctrl', ['$scope', "$filter", "$http", "ngTableParams", function ($scope, $filter, $http, ngTableParams) {
    ...
    $scope.genereteReport = function (data1, filtype) {
        ...
        if ($scope.filtype === "day") {
            ...
            url = serviceBase + "api/Comparison1/day?datefrom=" + $scope.dataforsearch.datefrom + "&dateto=" + $scope.dataforsearch.dateto + "&type=" + data1.type + "&ids=" + ids;
        }
        else if ($scope.filtype === "month") {
            $scope.frmt = "MMM YY";
            $scope.intervalType = "month";
            url = serviceBase + "api/Comparison1/month?datefrom=" + $scope.dataforsearch.datefrom + "&dateto=" + $scope.dataforsearch.dateto + "&type=" + data1.type + "&ids=" + ids;
        }
        else if ($scope.filtype === "year") {
            $scope.frmt = "YYYY";
            $scope.intervalType = "year";
            url = serviceBase + "api/Comparison1/year?datefrom=" + $scope.dataforsearch.datefrom + "&dateto=" + $scope.dataforsearch.dateto + "&type=" + data1.type + "&ids=" + ids;
        }
        $http.get(url)
    ...
    $scope.filtertype = function (filterData) {
        ...
        for (var i = 0; i < filterData.length; i++) {
            if ($scope.type === "1") {
                ...
                var activeRetailers = { x: new Date(filterData[i].dtt), y: filterData[i].activeBrands, name: filterData[i].name }
                OrderSummary.push(activeRetailers);
                titleText = "Brand's";
